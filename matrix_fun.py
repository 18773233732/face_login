...
class matrix:
    ...
    def EuclideanDistances(self, A, B):
        ...
        return ED.transpose()
    #
    def get_socres(self, A):
        # 设置每次处理的最大数据库记录数
        #如果数据库中记录太多时可分批进行处理
        maxlen = 128

        # fmysql = face_mysql.face_mysql()
        results = np.array(face_mysql.findall_facejson())
        ...
        for i in range(0, len(results), maxlen):
            pic_vectors = results[i:i + maxlen, 1]
            # 效率待优化，现在是每行处理
        ...
        # print(pic_min_names)
        # print(pic_min_scores)
        # print('################')
        # print(pic_min_uid)
        return pic_min_scores.tolist(), pic_min_uid
