...
with tf.Graph().as_default():
    ...
    def face_insert():
        ...
        images = image_array_align_data(
            img, image_path, pnet, rnet, onet, detect_multiple_faces=False)
        ...
        for j in range(0, len(emb_array)):
            ...
            last_id = face_mysql_instant.insert_facejson(filename_base + "_" + str(j),
                                                         ",".join(str(li) for li in emb_array[j].tolist()), uid)
...
def getTestData():
    return json.dumps({"data": "getTestData"})
